...
This array _mirrors_ the [Hook Chain](/pt-br/docs/hooks/concepts/chaining) installed on the account:
...
For more information see: [Reference Counting](/pt-br/docs/hooks/concepts/reference-counted-hook-definitions)

### Hook Defaults

When a `HookDefinition` is created it contains the initial [Parameters](/pt-br/docs/hooks/concepts/parameters), [Namespace](/pt-br/docs/hooks/concepts/namespaces) and [Grants](/pt-br/docs/hooks/concepts/grants) supplied by the user. These become the Hook Defaults. Any Hook referencing this Hook Definition will use these defaults _unless_ the SetHook Transaction that creates that reference explicitly overrides those defaults, or a subsequent Update Operation overrides them.
