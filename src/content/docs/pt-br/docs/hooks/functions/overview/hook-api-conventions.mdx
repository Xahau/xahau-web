...
Only two functions are allowed within a Hook: `hook()` and `cbak()`. Read about this [here](/pt-br/docs/hooks/concepts/compiling-hooks)

### Parameters

<Tabs>
<TabItem label="C">
All parameters passed to a Hook API must be one of: `uint32_t, int32_t, uint64_t, int64_t`. Typically these are pointers and lengths of buffers within the Hook's stack frame. Sometimes they are [Integer Encoded Floating Point Numbers (XFL)](/pt-br/docs/hooks/concepts/floating-point-numbers-xfl) or other data.
...
All parameters passed to a Hook API must be one of: `string, number[], bigint(xfl), object(json)`. Typically these are pointers and lengths of buffers within the Hook's stack frame. Sometimes they are [Integer Encoded Floating Point Numbers (XFL)](/pt-br/docs/hooks/concepts/floating-point-numbers-xfl) or other data.
...
All Hook APIs return a _signed integer_. Read about return codes here: [Return codes](/pt-br/docs/hooks/functions/overview/return-codes)
