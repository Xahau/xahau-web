...
As seções a seguir, [Contêiner Docker](#script-de-instalação-em-contêiner-docker) e [Instalação Local](#script-de-instalação-local), aproveitam scripts bash, que automatizam as seguintes etapas:
...
8. Criar um arquivo de serviço systemd (como o exemplo [abaixo](#exemplo-de-arquivo-de-serviço-systemd)) para executar o xahaud como um daemon.
...
Os repositórios Docker do Xahau (Mainnet e Testnet) contêm um script para automatizar o processo de instalação local, para que o xahaud possa ser executado fora do Docker. Esses scripts são semelhantes ao [Resumo da Instalação Linux](#processo-de-instalação-no-linux), pois criarão arquivos systemd, um usuário 'xahaud' e arquivos de configuração padrão. As URLs para esses scripts estão na tabela no final desta página. Para instalar localmente:
