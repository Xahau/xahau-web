...
An account-owned [ExportCommittee](/docs/protocol-reference/ledger-data/ledger-objects-types/export-committee) stores a canonical list of validator master public keys. When an [Export intent](/docs/protocol-reference/transactions/transaction-types/export) is admitted, every committee member must appear in the exact parent ledger's pre-NegativeUNL `ActiveValidators` set. If any member is absent, the intent returns `tecEXPORT_COMMITTEE_UNAVAILABLE`.
