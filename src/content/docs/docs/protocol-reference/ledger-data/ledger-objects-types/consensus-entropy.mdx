...
Standalone mode labels this synthetic entropy as tier 4 (`validator_full`) with count 20 and denominator 20. This lets `min_tier=4` hooks pass locally. Use it for local development only; real network ledgers can fall back to tier 1 when validator entropy is unavailable.
