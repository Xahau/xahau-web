...
description: >-
  ...
  validator commit/reveal, used by hooks via the entropy_cr_dice() and entropy_cr_random() APIs.
---
...
<Aside type="caution">
Tier 1 (`consensus_fallback`) entropy is deterministic and derivable from public inputs. It is grindable by transaction submitters, especially on quiet ledgers. Hooks handling high-value outcomes (lotteries, games) should require at least `min_tier=3` (validator quorum) in their `entropy_cr_dice()` / `entropy_cr_random()` calls — or `min_tier=4` (`validator_full`) to additionally reject any round in which an active validator withheld its reveal.
</Aside>
