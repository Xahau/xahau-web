...
Hooks consume this entropy through the `dice()` and `random()` hook API functions, and read the participation metadata (tier, reveal count, denominator) through `entropy_status()`.
