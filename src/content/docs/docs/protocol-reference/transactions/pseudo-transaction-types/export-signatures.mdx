...
| `EntropyContributors` | String | Blob | Exact-width LSB-first bitmap over the immutable ExportCommittee's canonical roster, identifying the positions represented in `ExportSigners`. Despite the shared field name, these are Export contributors, not RNG contributors. |
