...
A valid intent returns `tesSUCCESS` and creates an [ExportLatch](/docs/protocol-reference/ledger-data/ledger-objects-types/export-latch) keyed by `(Account, W)`, where W is the source Export transaction ID. Signature collection starts asynchronously after the admitting ledger validates.
...
| `temMALFORMED` | The operation forms are mixed or absent; a defined Export flag is used on the wrong form; committee bytes or digest are invalid; target authorization, Ticket, network, or reserved Memo rules fail; or an intent lacks its required outer deadline. |
| `temINVALID_FLAG` | `Flags` contains a bit outside `tfExportMask`. |
