...
Enables per-ledger [Consensus Entropy](/docs/features/consensus-entropy), allowing Hooks to call `dice()` and `random()` with explicit `min_tier` and `min_count` requirements. The ledger stores the selected digest, participant count, and entropy tier in the singleton [ConsensusEntropy ledger object](/docs/protocol-reference/ledger-data/ledger-objects-types/consensus-entropy).
