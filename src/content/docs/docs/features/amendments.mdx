...
Enables [Export transactions](/docs/protocol-reference/transactions/transaction-types/export), allowing a Xahau account or Hook to create a cross-chain transaction that the active validator set collectively multisigns. Successful exports produce an `ExportResult` in metadata (a reference to the validator signatures, which clients assemble into the submittable transaction); exports that carry an inner transaction also create a shadow ticket that latches the pending callback — not a permanent replay tombstone (see [Cross-Chain Export](/docs/features/export#shadow-tickets)).

##### ConsensusEntropy

Enables per-ledger [Consensus Entropy](/docs/features/consensus-entropy), allowing Hooks to call `dice()` and `random()` with explicit `min_tier` and `min_count` requirements. The ledger stores the selected digest, participant count, participation denominator, and entropy tier in the singleton [ConsensusEntropy ledger object](/docs/protocol-reference/ledger-data/ledger-objects-types/consensus-entropy).
