...
Enables [Export transactions](/docs/protocol-reference/transactions/transaction-types/export), allowing a Xahau account or Hook to create a cross-chain transaction that the active validator set collectively multisigns. Successful exports produce an `ExportResult` in metadata (a reference to the validator signatures, which clients assemble into the submittable transaction); exports that carry an inner transaction also create a shadow ticket keyed to the canonical target signing intent. Signature publication requires a ledger-anchored validator view whose original population fits the destination multisign cap (see [Cross-Chain Export](/docs/features/export#shadow-tickets)).

##### ConsensusEntropy

Enables per-ledger [Consensus Entropy](/docs/features/consensus-entropy), allowing Hooks to call `dice()` and `random()` with an explicit `min_tier` quality floor and to read the accepted result's tier, reveal count, and participation denominator via `entropy_status()`. The ledger stores the selected digest, participant count, participation denominator, contributor bitmap, and entropy tier in the singleton [ConsensusEntropy ledger object](/docs/protocol-reference/ledger-data/ledger-objects-types/consensus-entropy).
