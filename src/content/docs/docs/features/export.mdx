...
committee-wide share publication and creates a permanent witness. The required
fee scales deterministically with the selected committee size and serialized
target size, in addition to the normal transaction fee; use the server's fee
calculation rather than hard-coding the examples above. Committee setup,
committee deletion, and latch control do not initiate signing and retain the
ordinary transaction fee schedule.
