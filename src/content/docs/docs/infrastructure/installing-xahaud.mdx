...
---
When running a node, users will have to configure settings based on whether the node will run on the test network or the main network. When transitioning a single node from one network to the other, database files must be wiped (with the exception of wallet.db, which contains the server's identity credentials).

In addition to [building from scratch](../building-xahau), there are three ways to install xahaud: Docker, locally, and using a portable binary.
## Docker Container

To connect to the Xahau Mainnet you can use this docker container:

1. Clone the appropriate (Mainnet or Testnet) Xahau Docker GitHub repository. URLs are in the table at the bottom of this page. <code>git clone [repository-url]</code>
2. Navigate to the repository directory. <code>cd [repository-name]</code>
3. Run `./build`
4. Run `./up`
5. Commands and additional information are located in the README.md file in the repository.

## Local Install
The Xahau Docker containers repositories (mainnet and testnet) each contains a script to automate the local install process. The URLs for these scripts are contained in the table at the bottom of this page. To install locally:

1. Download and run the install script: <code>curl -sL [link-to-script] | bash</code>
...
3. Download and edit the appropriate (main or test network) configuration and trusted validators files using the links in the table below.
3. Run <code>./xahaud --net --conf xahaud.cfg</code> (replace 'xahaud.cfg') with the path to your downloaded configuration file. The validators' file path is defined in the configuration file and thus is not specified at runtime.
...
<tr><td>Network ID</td><td>21337</td><td>21338</td></tr>
<tr>
...
<tr><td>Binary xahaud Releases</td><td><a href="https://build.xahau.tech">https://build.xahau.tech</a></td><td>Same as Mainnet</td></tr>
