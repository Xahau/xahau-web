...
2. Download and edit the relevant configuration and trusted validators files (see the table below for links to Mainnet and Testnet configurations).
3. Change permissions on the downloaded binary, so it is executable: <code>chmod 555 [filename]</code>
5. Run the binary: <code>./[filename] --net --conf [path to xahaud.cfg]</code>
6. Verify xahaud is running: <code>./[filename] server_info</code>
