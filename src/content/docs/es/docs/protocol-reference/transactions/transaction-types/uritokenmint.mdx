...
title: URITokenMint
description: >-
  Una transacción URIToken Mint acuña un nuevo URIToken y asigna la propiedad
  a la cuenta especificada. El URIToken representa un activo digital único
  que puede usarse en diversas aplicaciones.
---
...
| `Destination` | String                                                                                                                             | AccountID           | _(Opcional)_ La dirección de la cuenta que puede comprar el URIToken acuñado.                  |
| `Amount`      | [Monto de Moneda](/es/docs/protocol-reference/data-types/currency-formats/#especificando-montos-de-moneda) | Amount              | _(Opcional)_ La cantidad de moneda que la cuenta quiere recibir a cambio del URIToken.         |
...
| -------------------- | ------------ | ------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| `tfBurnable`         | `0x00000001` | 1             | Permite al emisor (o a una entidad autorizada por el emisor) destruir el `URIToken` acuñado. (El propietario del `URIToken` _siempre_ puede hacerlo.) |
