...
title: CheckCreate
description: >-
  Crea un objeto Check en el ledger, que es un pago diferido que puede ser
  canjeado por su destino previsto. El emisor de esta transacción es el
  emisor del Check.
---
...
| `SendMax`        | [Monto de Moneda](/es/docs/protocol-reference/data-types/currency-formats/#specifying-currency-amounts) | Amount              | Monto máximo de moneda fuente que el Check puede debitar al emisor, incluyendo tarifas de transferencia en monedas que no son XAH. El Check solo puede acreditar al destino con la misma moneda (del mismo emisor, para monedas que no son XAH). Para montos que no son XAH, los nombres de campos anidados DEBEN estar en minúsculas. |
| `DestinationTag` | Number                                                                                                                             | UInt32              | _(Opcional)_ Etiqueta arbitraria que identifica la razón del Check, o un destinatario alojado al que pagar.                                                                                                                                                                                                 |
| `Expiration`     | Number                                                                                                                             | UInt32              | _(Opcional)_ Tiempo después del cual el Check ya no es válido, en \[segundos desde la Época Ripple]\[].                                                                                                                                                                                                     |
| `InvoiceID`      | String                                                                                                                             | Hash256             | _(Opcional)_ Hash arbitrario de 256 bits que representa una razón o identificador específico para este Check.                                                                                                                                                                                               |
