...
title: トランザクションメタデータ
---
トランザクションメタデータは、トランザクションが処理された後に追加されるデータの一部です。レジャーに含まれるトランザクションには、成功したかどうかに関係なくメタデータがあります。トランザクションメタデータは、トランザクションの結果を詳細に説明します。

:::caution
ここで説明する変更が最終的なものになるのは、トランザクションが検証済みレジャーバージョンにある場合だけです。
:::

<details>

<summary>メタデータの例</summary>

次のJSONオブジェクトは、[複雑な通貨間支払い](https://xrpcharts.ripple.com/#/transactions/8C55AFC2A2AA42B5CE624AEECDB3ACFDD1E5379D4E5BF74A8460C5E97EF8706B)のメタデータを示しています。
...
`AffectedNodes`配列には、このトランザクションが何らかの形で変更したレジャー内のオブジェクトの完全な一覧が含まれます。この配列の各エントリは、種類を示すトップレベルフィールドを1つ持つオブジェクトです。

* `CreatedNode`は、トランザクションがレジャー内に新しいオブジェクトを作成したことを示します。
* `DeletedNode`は、トランザクションがレジャーからオブジェクトを削除したことを示します。
* `ModifiedNode`は、トランザクションが既存のオブジェクトをレジャー内で変更したことを示します。

これらの各フィールドの値は、レジャーオブジェクトに加えられた変更を記述するJSONオブジェクトです。

#### CreatedNodeのフィールド

`CreatedNode`オブジェクトには、次のフィールドがあります。

| フィールド         | 値                  | 説明                                                                                                                                                       |
| ------------------ | ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `LedgerEntryType` | String              | 作成されたレジャーオブジェクトの種類。                                                                                                                 |
| `LedgerIndex`     | String - \[Hash]\[] | レジャーの状態ツリーにおけるこのレジャーオブジェクトのID。**注:** フィールド名は非常に似ていますが、これはレジャーインデックスとは**同じではありません**。 |
| `NewFields`       | Object              | 新しく作成されたレジャーオブジェクトの内容フィールド。どのフィールドが存在するかは、どの種類のレジャーオブジェクトが作成されたかによって異なります。                         |

#### DeletedNodeのフィールド

`DeletedNode`オブジェクトには、次のフィールドがあります。

| フィールド         | 値                  | 説明                                                                                                                                                       |
| ------------------ | ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `LedgerEntryType` | String              | 削除されたレジャーオブジェクトの種類。                                                                                                                 |
| `LedgerIndex`     | String - \[Hash]\[] | レジャーの状態ツリーにおけるこのレジャーオブジェクトのID。**注:** フィールド名は非常に似ていますが、これはレジャーインデックスとは**同じではありません**。 |
| `FinalFields`     | Object              | 削除される直前のレジャーオブジェクトの内容フィールド。どのフィールドが存在するかは、どの種類のレジャーオブジェクトが作成されたかによって異なります。     |

#### ModifiedNodeのフィールド

`ModifiedNode`オブジェクトには、次のフィールドがあります。

| フィールド           | 値                          | 説明                                                                                                                                                                                                                                                                              |
| ------------------- | --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `LedgerEntryType`   | String                      | 削除されたレジャーオブジェクトの種類。                                                                                                                                                                                                                                              |
| `LedgerIndex`       | String - \[Hash]\[]         | レジャーの状態ツリーにおけるこのレジャーオブジェクトのID。**注:** フィールド名は非常に似ていますが、これはレジャーインデックスとは**同じではありません**。                                                                                                                               |
| `FinalFields`       | Object                      | このトランザクションによる変更を適用した後のレジャーオブジェクトの内容フィールド。どのフィールドが存在するかは、どの種類のレジャーオブジェクトが作成されたかによって異なります。これは、ほとんどのレジャーオブジェクト型が持つ`PreviousTxnID`フィールドと`PreviousTxnLgrSeq`フィールドを除外しています。 |
| `PreviousFields`    | Object                      | このトランザクションの結果として変更された、オブジェクトのすべてのフィールドの変更前の値。トランザクションがオブジェクトにフィールドを _追加しただけ_ の場合、このフィールドは空のオブジェクトになります。                                                                                                 |
| `PreviousTxnID`     | String - \[Hash]\[]         | _(省略される場合があります)_ このレジャーオブジェクトを変更した直前のトランザクションの \[識別ハッシュ]\[]。`PreviousTxnID` フィールドを持たないレジャーオブジェクト型では省略されます。                                                                                                        |
| `PreviousTxnLgrSeq` | Number - \[Ledger Index]\[] | _(省略される場合があります)_ このレジャーオブジェクトを変更した直前のトランザクションを含むレジャーバージョンの \[レジャーインデックス]\[]。`PreviousTxnLgrSeq` フィールドを持たないレジャーオブジェクト型では省略されます。                                                                          |

**注:** 変更されたレジャーオブジェクトに`PreviousTxnID`フィールドと`PreviousTxnLgrSeq`フィールドがある場合、トランザクションは常にそれらをトランザクション自身の識別ハッシュと、トランザクションを含んだレジャーバージョンのインデックスで更新します。ただし、これらのフィールドの新しい値は`ModifiedNode`オブジェクトの`FinalFields`には含まれず、以前の値は入れ子の`PreviousFields`オブジェクトではなく`ModifiedNode`オブジェクトのトップレベルに一覧されます。

### delivered_amount

`[Payment transaction][]`の`Amount`は`Destination`に届ける金額を示すため、トランザクションが成功していれば、部分支払いでない限り宛先はその金額を受け取っています。(その場合、`Amount`までの任意の正の金額が届いている可能性があります。)`Amount`フィールドを信頼するかどうかを判断する代わりに、メタデータの`delivered_amount`フィールドを使って、実際に宛先へ届いた金額を確認してください。

`rippled`サーバーは、成功したすべてのPaymentトランザクションのJSONトランザクションメタデータに`delivered_amount`フィールドを提供します。このフィールドは通常の通貨金額と同じ形式です。ただし、次の両方の条件を満たすトランザクションでは、送信額は利用できません。

* 部分支払いである

トランザクションが部分支払いの場合、`delivered_amount`には実際の金額の代わりに文字列値`unavailable`が入ります。この場合、実際に届いた金額はトランザクションメタデータの`AffectedNodes`を読むことでしか判定できません。

**注:** `delivered_amount`フィールドはリクエスト時にオンデマンドで生成されるため、トランザクションメタデータのバイナリ形式には含まれず、トランザクションメタデータのハッシュを計算する際にも使用されません。
