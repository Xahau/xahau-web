...
2. `env`ディレクトリにPython3の仮想環境(venv)を作成します: `python3 -m venv env`

3. 仮想環境を有効にします: `source ./env/bin/activate`

4. 最新のソフトウェアバージョンを利用できるようにpipを更新します: `pip install --upgrade pip`

5. Conan2をインストールします: `pip install conan`

6. 新しいConan2プロファイルを作成します (まだ作成していない場合): `conan profile detect`
