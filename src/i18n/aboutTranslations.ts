export const aboutTranslations = {
  ...
  ja: {
    ...
    page_subtitle:
      'アカウントベースのプログラマビリティを持つエンタープライズL1ブロックチェーン',
    chip1: '2023年10月ローンチ',
    chip2: '20万以上のアカウント',
    chip3: '~4秒で決済',
    chip4: '1万トランザクション/レジャー',
    ...
    act1_h3_line2: '簡単な説明',
    act1_para1:
      'Xahauは、本格的なプログラマビリティのために構築されたL1ブロックチェーンです。実績あるレジャー技術と、Hooksと呼ばれるスマートコントラクトへの強力かつ独自のアプローチを組み合わせています。',
    act1_para2:
      'この技術は約4秒でトランザクションを決済し、手数料は1米ドルセントの端数程度の低水準です。1レジャーあたり最大10,000トランザクションを処理でき、設計上、本質的にグリーンでエネルギー効率の高いブロックチェーンです。',
    act1_para3:
      'Xahauは、プロトコルネイティブの分散型取引所での通貨の発行・取引、ネイティブな非代替性トークン、Remitトランザクターによるアトミックなマルチアセットトランザクション、そしてXahauのアカウントベースのスマートコントラクトシステムであるHooksによる無限のDeFiの可能性といったネイティブ機能を提供します。',
    act1_para4:
      'Hooksはアカウントに直接存在し、支払いの受信・送信・ルーティングといったトランザクションイベントに応答します。当初はC言語で開発されていましたが、現在はWebAssemblyに対応するあらゆる言語をサポートしています。JavaScriptによるHooksサポート（JSHooks）はセキュリティ監査を通じて前進しており、世界中の数千万人の開発者がプログラマブルファイナンスに手が届くようになっています。',
    ...
    act2_para1:
      'Xahauは実証済みの本番環境対応のテクノロジーです。アカウントベースのプログラマビリティ（Hooks）、エスクローやペイメントチャネルなどのネイティブ機能での発行通貨への平等なサポート、そして複雑な複数ステップの操作を単一の分割不可能なユニットとして実行できるアトミックトランザクション層を加えて強化された、XRPレジャーコードベースの進化版です。',
    act2_para2:
    ...
    act3_h3_line1: 'XAH：Xahauの',
    act3_h3_line2: 'ネイティブ通貨',
    act3_para1:
      'Xahauのネイティブ通貨はXAHです。積極的な参加を報酬として返すように設計されたインフレ資産です。すべてのアカウントは、ジェネシスアカウントのHookと対話することで月次残高報酬を請求でき、毎月アカウント残高の4%を獲得できます。',
    act3_para2:
